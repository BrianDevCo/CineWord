...
export default function Navbar() {
  ...
  return (
    ...
    >
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {links.map((l) => (
              <a
                key={l.label}
                href={l.href}
                className="font-heading text-sm tracking-widest uppercase text-gray-300 hover:text-[#CC1244] transition-colors duration-200"
              >
                {l.label}
              </a>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-3">
            {user ? (
              <div className="flex items-center gap-2">
                <Link
                  href="/cuenta"
                  className="flex items-center gap-2 font-heading text-xs tracking-widest px-4 py-2.5 rounded-sm border border-white/20 text-gray-300 hover:border-[#CC1244] hover:text-[#CC1244] transition-all"
                >
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <svg className={`w-3 h-3 text-gray-500 transition-transform ${userMenuOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center gap-2">
                <Link
                  href="/cuenta/login"
                  className="font-heading text-xs tracking-widest px-4 py-2.5 rounded-sm border border-white/20 text-gray-300 hover:border-[#CC1244] hover:text-[#CC1244] transition-all"
                >
                  INGRESAR
                </Link>
                <a
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="lg:hidden border-t border-white/10 bg-black/98 px-4 py-6 flex flex-col gap-4">
          {links.map((l) => (
            <a
              key={l.label}
              href={l.href}
              onClick={() => setMenuOpen(false)}
              className="font-heading text-lg tracking-widest uppercase text-gray-300 py-1 hover:text-[#CC1244] transition-colors"
            >
              {l.label}
            </a>
          ))}
            ...
            {user ? (
              <>
                <Link
                  href="/cuenta"
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 font-heading text-sm tracking-widest text-gray-300 py-1 hover:text-[#CC1244] transition-colors"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M2 6a2 2 0 012-2h16a2 2 0 012 2v2a1 1 0 010 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2v-6a1 1 0 010-2V6z" />
                  </svg>
                ...
                <Link
                  href="/cuenta"
                  onClick={() => setMenuOpen(false)}
                  className="font-heading text-sm tracking-widest text-gray-400 py-1 hover:text-[#CC1244] transition-colors"
                >
              ...
              <Link
                href="/cuenta/login"
                onClick={() => setMenuOpen(false)}
                className="font-heading text-sm tracking-widest text-gray-300 py-1 hover:text-[#CC1244] transition-colors"
              >
