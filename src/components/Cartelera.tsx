...
export default function Cartelera() {
  ...
  const rest = activeFilter === "Todas"
    ? movies.slice(1)
    : movies.filter((m) => m.filterGenres.includes(activeFilter));
              ...
              <div className="absolute inset-0 flex items-end p-8 lg:p-12">
                <div className="flex gap-6 items-end max-w-3xl">
                  ...
                  <div className="flex flex-col gap-3">
                    <span className={`${featured.badgeColor} text-white font-heading text-xs px-3 py-1 rounded-sm tracking-widest w-fit animate-pulse-red`}>
                      {featured.badge}
                    </span>
                    <h3 className="font-heading text-4xl lg:text-5xl font-bold text-white tracking-wider leading-tight">
                      {featured.title}
                    </h3>
                    <p className="text-gray-400 font-body text-sm hidden sm:block">{featured.genre} · {featured.duration}</p>
              ...
              <Link key={movie.id} href={`/pelicula/${movie.id}`}>
                <div className="group flex flex-col rounded-xl overflow-hidden border border-white/5 hover:border-white/20 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-black/60 cursor-pointer">
                  ...
                  <div className="relative aspect-[2/3] overflow-hidden">
                    ...
                    {/* Badge */}
                    <span className={`absolute top-2 left-2 ${movie.badgeColor} text-white font-heading text-[10px] px-1.5 py-0.5 rounded-sm tracking-widest`}>
                      {movie.badge}
                    </span>
                    {movie.formats.includes("VIP") && (
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      <p className="text-gray-400 text-[10px] font-body">{movie.genre}</p>
