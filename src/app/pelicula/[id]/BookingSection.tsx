...
function buildWeek(): CalendarDay[] {
  return Array.from({ length: 2 }, (_, i) => {
    const d = new Date();
    d.setDate(d.getDate() + i);
    return {
      label:     i === 0 ? "Hoy" : "Mañana",
...
export default function BookingSection({ movie }: Props) {
  ...
  const isWeekend  = dayData ? (dayData.dayOfWeek === 0 || dayData.dayOfWeek === 6) : false;
  const timesForDay = isWeekend ? movie.weekSchedule.weekend : movie.weekSchedule.weekday;
  const availableFormats = timesForDay.map(t => t.format);
  const timesForFormat   = timesForDay.find(t => t.format === selectedFormat)?.horarios ?? [];

  const total = [...selectedSeats].reduce(
    (acc, id) => acc + (VIP_ROWS.includes(id[0]) ? PRICE_VIP : PRICE_REGULAR), 0
  );
  ...
  useEffect(() => {
    if (timerSeconds === 0) { setTimedOut(true); setSelectedSeats(new Set()); setStep(4); setTimerSeconds(null); }
  }, [timerSeconds]);
      ...
      <div className="flex flex-col items-center gap-6 py-16 text-center">
        ...
        <div className="bg-[#111] border border-white/10 rounded-xl p-6 max-w-sm w-full text-left flex flex-col gap-2">
          ...
          {[
            ...
            { label: "Fecha",     value: dayData ? `${dayData.label} · ${dayData.date}` : "" },
      ...
      <div className={`flex flex-col gap-4 ${step > 1 ? "mb-6 pb-6 border-b border-white/10" : "mb-10"}`}>
        <div className="flex items-center justify-between">
          ...
          {step > 1 && selectedDay !== null && (
            <button onClick={() => { stopTimer(); setStep(1); setSelectedFormat(null); setSelectedTime(null); setSelectedSeats(new Set()); }}
        ...
        {step === 1 ? (
            ...
            {week.map((day, i) => {
              ...
              return (
                <button
                  key={i}
                  onClick={() => pickDay(i)}
                  className={`group flex flex-col items-center gap-1.5 flex-1 rounded-xl px-6 py-5 border transition-all hover:border-[#CC1244]/50 hover:bg-[#1a1a1a] ${
                    isWknd ? "bg-[#111] border-yellow-500/20" : "bg-[#111] border-white/10"
                  }`}
                >
                  <span className={`font-heading text-[10px] tracking-widest uppercase ${
                    isWknd ? "text-yellow-500/70" : "text-gray-600"
                  }`}>{day.date}</span>
                  <span className="font-heading text-2xl font-bold text-white group-hover:text-[#CC1244] transition-colors">
                    {day.label}
                  </span>
                  {isWknd && (
                    <span className="text-[9px] font-heading tracking-widest text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded-sm">
                      MÁS FUNCIONES
                    </span>
                  )}
                  {isTueWed && !isWknd && (
                    <span className="text-[9px] font-heading tracking-widest text-[#CC1244] bg-[#CC1244]/10 px-2 py-0.5 rounded-sm">
                      50% PROMO
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-500 font-body text-sm">{dayData?.date}</span>
            {isWeekend && (
              <span className="text-yellow-500 font-heading text-[10px] tracking-widest border border-yellow-500/30 px-2 py-0.5 rounded-sm">
                FIN DE SEMANA
              </span>
            )}
          </div>
      ...
      {step >= 2 && (
        <div className={`flex flex-col gap-4 ${step > 2 ? "mb-6 pb-6 border-b border-white/10" : "mb-10"}`}>
          <div className="flex items-center justify-between">
            ...
            {step > 2 && (
              <button onClick={() => { stopTimer(); setStep(2); setSelectedTime(null); setSelectedSeats(new Set()); }}
                className="text-[#CC1244] font-heading text-xs tracking-widest hover:underline">CAMBIAR</button>
            )}
          </div>
          {step === 2 ? (
                  ...
                  <div key={row} className="flex items-center gap-2 justify-center">
                    ...
                    <div className="flex gap-1.5">
                      {Array.from({ length: COLS }, (_, i) => {
                        const id = `${row}${i + 1}`;
                        return (
                          <button key={id} onClick={() => toggleSeat(id)} title={OCCUPIED.has(id) ? "Ocupado" : id}
          ...
          {timerSeconds !== null && (
            <div className={`rounded-xl border px-5 py-4 flex flex-col gap-3 transition-all ${
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 font-body text-xs">
                      {isCritical ? "Los asientos se liberarán en segundos si no completas el pago" : "Completa el pago antes de que expire la reserva"}
                    </p>
                  </div>
                </div>
              ...
              <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-1000 ${barColor} ${isCritical ? "animate-pulse" : ""}`}
                  style={{ width: `${barPct}%` }} />
              </div>
            </div>
          )}
