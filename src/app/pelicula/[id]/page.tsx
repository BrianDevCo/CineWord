    ...
    <main className="bg-[#0a0a0a] min-h-screen">
      ...
      <section className="relative min-h-[70vh] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 pt-32 w-full flex flex-col sm:flex-row items-end gap-8">
          {/* Poster */}
          <div className="shrink-0 w-40 h-60 lg:w-52 lg:h-78 rounded-xl overflow-hidden relative shadow-2xl border border-white/10 hidden sm:block"
            style={{ height: "clamp(240px, 20vw, 312px)" }}>
            <Image src={movie.poster} alt={movie.title} fill className="object-cover" sizes="208px" />
          </div>

          {/* Info */}
          <div className="flex flex-col gap-4 flex-1">
            <div className="flex items-center gap-3 flex-wrap">
              <span className={`${movie.badgeColor} text-white font-heading text-xs px-3 py-1 rounded-sm tracking-widest`}>
                {movie.badge}
              </span>
              <span className="text-gray-400 text-sm font-body">{movie.genre}</span>
              <span className="text-gray-600">•</span>
              <span className="text-gray-400 text-sm font-body">{movie.duration}</span>
              <span className="text-gray-600">•</span>
              <span className="text-gray-400 text-sm font-body">{movie.rating}</span>
            </div>
            ...
            <div className="flex gap-2">
              {movie.formats.map((f) => (
                <span key={f} className={`font-heading text-xs tracking-widest px-3 py-1.5 rounded-sm border ${
                  f === "VIP"
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <p className="text-gray-400 font-body leading-relaxed">{movie.synopsis}</p>

            {/* Ficha técnica */}
            <div className="flex flex-col gap-3 border-t border-white/10 pt-5">
              {[
                { label: "Director", value: movie.director },
                { label: "Reparto", value: movie.cast.join(", ") },
                { label: "Duración", value: movie.duration },
                { label: "Clasificación", value: movie.rating },
                { label: "Formatos", value: movie.formats.join(", ") },
              ].map((row) => (
          ...
          <div className="lg:col-span-3 flex flex-col gap-4">
            ...
            {/* Trailer embed — thumbnail con play button */}
            <a
              href={`https://www.youtube.com/results?search_query=${encodeURIComponent(movie.trailerSearch)}`}
              target="_blank"
            ...
            >
              {hasBackdrop ? (
                <Image src={movie.backdrop!} alt="Trailer" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 60vw" />
              ) : (
                <Image src={movie.poster} alt="Trailer" fill className="object-cover object-top" sizes="(max-width: 1024px) 100vw, 60vw" />
              )}
              {/* Overlay oscuro */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="mb-10">
            ...
            <p className="text-gray-500 font-body text-sm mt-1">
              Funciones disponibles para los próximos 2 días. Selecciona fecha, formato y horario.
            </p>
          </div>
              ...
              >
                <div className="relative aspect-[2/3] overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    <p className="text-gray-400 text-[10px] font-body">{m.genre}</p>
                    <p className="font-heading text-sm font-bold text-white leading-tight line-clamp-2">{m.title}</p>
                  </div>
                </div>
              </Link>
            ))}
